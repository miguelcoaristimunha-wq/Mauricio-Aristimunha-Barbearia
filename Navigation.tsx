                    ...
                    {tabs.map(tab => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex-1 flex flex-col items-center justify-center h-full transition-all ${isActive ? 'text-gold' : 'text-gray-400 dark:text-gray-500'
                                    }`}
                            >
                                <span className={`material-icons-round text-2xl transition-transform ${isActive ? 'scale-110' : ''}`}>
                                    {tab.icon}
                                </span>
                                <span className={`text-[9px] font-bold uppercase tracking-widest mt-1 ${isActive ? 'text-gold' : ''}`}>
                                    {tab.label}
                                </span>
                                <div className={`w-1 h-1 bg-gold rounded-full mt-1 transition-opacity ${isActive ? 'opacity-100' : 'opacity-0'}`}></div>
                            </button>
                        );
                    })}
                </div>
